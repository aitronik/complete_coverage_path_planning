...
MAX_STEPS = 4000 
BASE_MAP = (_img != 255).astype(np.int8)
...
class VecNormalizeCheckpointCallback(BaseCallback):
    def __init__(self, env, save_freq, save_path, name_prefix="vecnormalize_checkpoint_50M_", verbose=0):
...
class GraphBasedEnv(Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        # MAX_STEPS = 2*FREE_CELLS + self.visited
    ...
    def _obs(self):
        return (self.state[None, ...].astype(np.float32))
...
def train(total_steps: int = TOTAL_TIMESTEPS, model_name_load: str = None, model_name_save: str = DEFAULT_MODEL_NAME):
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_checkpoint_50M_"
    )
...
def inference_video(model_name_load: str = DEFAULT_MODEL_NAME, video_path="inference_video.avi", fps=10):
    vec_env = DummyVecEnv([make_env(0)])
    env = VecNormalize.load(DEFAULT_MODEL_PATH + model_name_load + "_vecnormalize.pkl", vec_env)
    env.training = False
    env.norm_reward = False
